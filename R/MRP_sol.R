#' MRP_sol: Mars Rover Photos - Querying by Martian sol
#'
...
#' @param sol Integer. sol (ranges from 0 to max found in endpoint). 1000 as default.
...
MRP_sol <- function(key = Sys.getenv("NASA_TOKEN"), sol = 1000, camera = "all", page = 1, type = "curiosity", download_image = FALSE){
  ...
  if (camera == "all"){
    url <- paste(url, type, "/photos?sol=", sol, "&api_key=", key, "&page=", page, sep = "")
  }else{
    url <- paste(url, type, "/photos?sol=", sol, "&api_key=", key, "&camera=", camera, "&page=", page, sep = "")
  }
  response <- GET(url)
