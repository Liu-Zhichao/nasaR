#' DONKI_SEP: Space Weather Database Of Notifications, Knowledge, Information - Solar Energetic Particle (SEP)
#'
#' Get access to the data of Solar Energetic Particle (SEP).
#'
...
#' @param start_date Date. Starting UTC date for SEP search. 30 days prior to current UTC date as default.
#' @param end_date Date. Ending UTC date for SEP search. Current UTC date as default.
#' @return Data of SEP.
#' @examples
...
DONKI_SEP <- function(key = Sys.getenv("NASA_TOKEN"), start_date = end_date - 30, end_date = lubridate::today(tzone = "UTC")){
  library(tidyr)
  library(httr)
  response <- "https://api.nasa.gov/DONKI/SEP?" %>%
